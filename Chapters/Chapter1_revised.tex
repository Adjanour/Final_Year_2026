...
\begin{enumerate}[label=\roman*.]
  ...
  \item to evaluate DCA-Trie v1 and v2 against the GCR baseline and an unconstrained chain-of-thought (CoT) baseline on WebQSP and CWQ, measuring Hits@1, F1, structural faithfulness, SIR, and average trie size, with results stratified by question hop depth; and
  \item to demonstrate DCA-Trie in practice through an interactive prototype system — either a web-based or command-line QA interface — allowing users to submit complex natural language questions and receive fact-grounded reasoning chains, showing the framework's value beyond benchmark evaluation.
\end{enumerate}
