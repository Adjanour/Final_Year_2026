...
\begin{enumerate}
  ...
  \item \textbf{Inductive Reasoning Layer (Unmodified):} After the constrained beam search produces $K$ top reasoning paths, GPT-4o-mini analyses these paths at the same time to synthesize the final natural language answer.
\end{enumerate}
...
The semantic relevance scorer implements the transition from a static oracle $f(\mathcal{G}, \mathcal{E}_q)$ to a context-aware oracle $f(\mathcal{G}, \mathcal{E}_q, q, y_{<t})$. As illustrated by the complete workflow in Figure~\ref{fig:scorer}, it takes a candidate KG path $p$, the question $q$, and the partial generation $y_{<t}$, and returns a relevance score in $[0,1]$.
...
A candidate path is admitted into $\mathcal{W}_{\text{val}}^{\text{DCA}}(t)$ if and only if $\text{score}(p, q, y_{<t}) \geq \tau$, where $\tau$ is the admission threshold. This binary decision integrates the semantic score into the logit-masking framework.
...
Algorithm~\ref{alg:v1} runs in two separate phases: an offline filtering phase and an online generation phase.

To explain how the mechanism works, the process unfolds in the following sequence:
