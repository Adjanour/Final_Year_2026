...
This chapter formalises the permissiveness problem identified in Chapter~2 and presents the Dynamic Context-Aware Trie (DCA-Trie) framework as a solution. It defines the upgraded oracle specification and the Semantic Irrelevance Ratio (SIR), a diagnostic metric that measures constraint quality independently of answer accuracy. The chapter traces the research evolution from a cosine-similarity scorer through a decomposed product score to the final symbolic TypeOracle, then describes the static pre-filtering design of v1 and the step-wise dynamic expansion of v2. It closes with the baseline configurations and evaluation protocol that govern the experimental work in Chapter~4.
...
As established in Chapter~2, current graph-constrained frameworks (specifically GCR \citep{luo2025-graph-constrained-reasoning} and DoG \citep{li2024-decoding-on-graphs}) rely on a deterministic oracle that looks only at the fixed graph $\mathcal{G}$ and the starting entities $\mathcal{E}_q$. Whether this oracle is built before decoding or expanded on the fly, the set of valid tokens at any step $t$ remains locked to structural logic:
\begin{equation}
...
Two requirements are in tension here. Structural validity ensures the LLM only generates tokens that correspond to real graph edges; current models do this well. Contextual relevance is the open problem: ensuring the LLM only generates tokens that logically connect to the specific question. Current models do not enforce this. The gap between structural compliance and semantic usefulness is the \textit{permissiveness problem}.
...
\begin{enumerate}
  \item \textbf{Structural Faithfulness (Condition~F):} Every candidate token $v \in \mathcal{W}_{\text{val}}^{\text{DCA}}(t)$ must correspond to a valid prefix of a path $p \in \mathcal{G}$ at every step $t$. This preserves the 100\% structural faithfulness guarantee achieved by GCR and DoG.

  \item \textbf{Semantic Relevance Reduction (Condition~R):} The Semantic Irrelevance Ratio (SIR) under DCA-Trie must be lower than the corresponding SIR under GCR when averaged over the evaluation corpus. Using SIR as defined in Section~\ref{sec:ch3sir}, the condition formalised in Equation~\eqref{eq:sir_condition} is
        \begin{equation}
...
DCA-Trie focuses on the constraint oracle layer, so its architecture directly builds on the baseline pipeline from Graph-Constrained Reasoning (GCR) \citep{luo2025-graph-constrained-reasoning}. This reuse isolates the experimental effects of the proposed oracle. The pipeline has four layers:

\begin{enumerate}
  \item \textbf{Entity Linking (Unmodified):} Identifies the core query entities $\mathcal{E}_q$ from the question $q$. These dictate where downstream graph exploration begins.

  \item \textbf{Constraint Oracle (DCA-Trie Contribution):} Controls which tokens the LLM can generate. In GCR, this layer creates a KG-Trie of structurally reachable paths within $L$ hops. DCA-Trie modifies this layer by applying symbolic ontology checks through a TypeOracle, via either static pre-filtering (v1) or dynamic step-wise expansion (v2), so that admitted paths remain graph-valid and type-consistent.

  \item \textbf{Constrained Decoding (Unmodified):} During autoregressive generation, the decoding layer intercepts the LLM logit distribution and applies a binary mask retrieved from the oracle layer. The LLM is restricted to tokens that are currently valid under the trie.

  \item \textbf{Inductive Reasoning (Unmodified):} After constrained decoding produces candidate reasoning paths, the answer synthesis component produces the final natural language answer.
\end{enumerate}
...
Current performance markers for knowledge graph decoding, such as Hits@1, F1, and structural faithfulness ratios, focus on output instead of process. They show whether a framework found the correct answer and stayed within the graph, but not how it reached that answer. A model may solve the correct gold path while wandering through a large number of irrelevant paths that waste resources.

To assess how permissive the oracle is, independently of final answer accuracy, this thesis presents a diagnostic metric called the Semantic Irrelevance Ratio (SIR).

\subsection{Formal Definition of SIR}

Suppose $\mathcal{P}(q, t)$ represents all paths that the constraint oracle allows at decoding step $t$ for question $q$. An individual candidate path $p \in \mathcal{P}(q, t)$ is structurally valid, but it fails the semantic stringency test if its terminal entity type is incompatible with the inferred answer types, or if any edge in the path violates the property range constraint:
\begin{equation}
...
The step-level SIR for question $q$ at decoding step $t$ is then the fraction of admitted paths that are irrelevant:
\begin{equation}
...
Corpus-level SIR averages over all questions and all valid decoding steps up to depth $L$:
\begin{equation}
...
By definition, $\overline{\text{SIR}} \in [0,1]$. A value near 1 indicates that most admitted paths are irrelevant, so the model faces a large and noisy search space. A value near 0 indicates that the constraint set is tightly aligned with question semantics. Prior constrained frameworks such as GCR and DoG tend to show high SIR, especially at larger hop depths where path counts increase rapidly. DCA-Trie targets lower SIR by filtering semantically weak paths while keeping the structurally valid ones. Chapter~4 compares SIR across WebQSP and CWQ.
...
The semantic relevance mechanism went through three designs before arriving at the final symbolic TypeOracle. The initial cosine-similarity scorer~(Approach~1) embedded each candidate path and scored it against the question. It failed on four counts: semantic collapse (a single score cannot distinguish type-match from topical overlap), encoder cost (thousands of forward passes per question), threshold sensitivity ($\tau = 0.25$ produced empty path sets for 84\% of WebQSP queries), and non-determinism from floating-point arithmetic. The decomposed product score~(Approach~2) split the cosine score into relational, entity-type, and trajectory components. The entity-type component was already a hard binary gate rather than a continuous score, an early step toward the fully symbolic design, but the relational and trajectory components still required encoder forward passes. Approach~2 improved interpretability but retained both the encoder dependency and the threshold. The final TypeOracle~(Approach~3) eliminates the encoder and the threshold entirely, using only $O(1)$ set lookups over the KG's own ontology metadata.
...
\begin{enumerate}
  \item \textbf{Answer Type Gate} ($\rho_e$, terminal hop only): Applied at the final hop of a candidate path. It checks whether the candidate answer entity's type matches what the question asks for. A question beginning with ``who'' infers person-type entities as valid answers.

  \item \textbf{Property Range Gate} ($\rho_r$, every hop): Applied at every edge in the path. It checks whether the tail entity's type is compatible with the relation's declared range in the ontology. If a relation expects a location as its range, then only entities typed as locations are admitted when type information is available.
\end{enumerate}

The combined admission check for a candidate edge $(r, e')$ at hop $h$, formalised in Equation~\eqref{eq:admissible}, is:
\begin{equation}
...
The gates operate on discrete set membership rather than continuous similarity values, as defined in Equations~\eqref{eq:type_gate} and \eqref{eq:range_gate}:
\begin{align}
...
Both gates default to True when schema metadata is absent. This ensures structural faithfulness is never compromised and the TypeOracle never produces empty path sets. The design is threshold-free: no continuous similarity score requires tuning.
...
Algorithm~\ref{alg:v1} follows four stages. First, the system pattern-matches question words against predefined patterns to infer the answer types $\mathcal{T}(q)$. Second, standard BFS maps out every possible graph path from the question entities up to maximum depth $L$, matching the baseline GCR approach. Third, the oracle checks whether every edge respects the relation's declared range and whether the terminal entity type matches the inferred answer types, keeping only paths that pass all gates. Fourth, the remaining paths are compiled into a static prefix tree, which the LLM queries at each token step during decoding.
...
Architecturally, v2 follows the step-wise traversal pattern in DoG \citep{li2024-decoding-on-graphs} but adds symbolic TypeOracle gating to each local expansion. After each committed entity $e_t$, the trie expands only to structurally valid neighbours that also satisfy the active type and range gates, as formalised in Equation~\eqref{eq:v2expansion}:
\begin{equation}
...
\end{equation}
The expansion procedure has four steps: (1)~initialize a minimal trie containing only the root question entities; (2)~at each decoding step, look up valid tokens from the current trie and apply logit masking; (3)~when the LLM commits an entity token, fetch all structurally valid neighbours from the knowledge graph and apply range\_gate and type\_gate to each candidate; (4)~add candidates passing both gates to the trie. Relation-token steps do not trigger expansion.

At each entity commitment, the expansion loop examines all $D$ neighbours, applying one $O(1)$ set check per neighbour. Over $T_e$ commitments, the total expansion cost is $O(T_e \cdot D \cdot L)$, versus v1's offline $O(|\mathcal{P}| \cdot L)$. In typical WebQSP questions $T_e \cdot D$ is smaller than $|\mathcal{P}|$ (a 2-hop question commits to 2 entities, each with $D \approx 20$ neighbours). The practical trade-off is not the per-step oracle cost but the trie-rebuild frequency: each rebuild modifies the trie structure that the LLM's prefix-constrained decoding depends on, introducing instability that static pre-filtering avoids.

\subsection{Algorithm for DCA-Trie v2}

Algorithm~\ref{alg:v2} formalises the step-wise expansion procedure. The trie starts minimal and grows only at entity-commitment steps, so that at every decoding step the set of valid tokens remains both graph-valid and type-consistent.
...
The key structural difference from v1 is in lines 8--16 of Algorithm~\ref{alg:v2}. Expansion is triggered only when the model commits an entity token, not at every step. At that moment, all structurally valid neighbours of $e_t$ are fetched from the knowledge graph and passed through the range and type gates; only candidates that pass both are added to the trie. At relation-token steps the trie is held constant, so no gate evaluation occurs. This concentrates computation at entity decision points while keeping per-step overhead predictable.
...
Formally, let $\mathcal{F}_t$ be the set of frontier nodes the live beams have committed to at step $t$. The trie at step $t$, defined in Equation~\eqref{eq:v3expansion}, is
\begin{equation}
...
\begin{itemize}
  ...
  \item \textbf{DCA-Trie v1:} Static symbolic filtering (Section~\ref{sec:ch3v1}).
  \item \textbf{DCA-Trie v2:} Dynamic step-wise expansion (Section~\ref{sec:ch3v2}).
  ...
  \item \textbf{DCA-Trie v3:} Lazy constrained decoding (Section~\ref{sec:ch3v3}).
\end{itemize}
...
Evaluation uses the RoG-webqsp test split (1,628 questions, 1--2 hops) over Freebase~\citep{bollacker2008freebase, yih2016websqp}. Because every method re-uses the same LLM weights and decoding settings, the paired comparison is run on a fixed 300-question subset sampled with seed~42; all conditions decode the identical question set, so differences are attributable to the constraint-oracle design alone. The full test set is used for the GCR reproduction study (Section~\ref{sec:ch4repro}) and for the gate-decomposition analysis at index length $L{=}4$. The pattern is verified on a CWQ subset (2--4 hops). A 100-question validation split is used only for gate statistics. The primary metrics are Hits@1, F1, Path Reduction, False Negative Rate (FNR), and the Semantic Irrelevance Ratio (SIR) of Section~\ref{sec:ch3sir}. All experiments run on a single NVIDIA GeForce RTX 4090 using \modelname{} in bfloat16 precision via HuggingFace Transformers~\citep{wolf2020transformers}, with a 120-second per-sample timeout.
