...
Four configurations are evaluated, all using the same \modelname{} backbone and matched decoding settings (beam width $k{=}10$, group-beam search). This isolates differences to the constraint-oracle design:
...
Evaluation uses the RoG-webqsp test split (1,628 questions, 1--2 hops) over Freebase~\citep{bollacker2008freebase, yih2016websqp}, with the resulting pattern verified on a CWQ subset (2--4 hops). A 100-question validation split is used only for gate statistics. The primary metrics are Hits@1, F1, Path Reduction, and False Negative Rate (FNR). All experiments run on a single NVIDIA GeForce RTX 4090 using the \modelname{} model in bfloat16 precision via HuggingFace Transformers~\citep{wolf2020transformers}. Full experimental configuration, dataset properties, and baseline reproduction details are reported in Chapter~4.
