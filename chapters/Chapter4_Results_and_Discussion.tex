...
\begin{table}[H]
  \centering
  \caption[Generation Configuration]{Generation parameters used across all experiments.}
...
\begin{table}[H]
  \centering
  \caption[Dataset Properties]{Properties of the evaluation datasets.}
  \label{tab:datasets}
...
\begin{table}[H]
  \centering
  \caption[Beam vs.\ Greedy Decoding]{Beam search vs.\ greedy decoding across methods.}
...
\begin{table}[H]
  \centering
  \caption[Gate Ablation]{TypeOracle gate ablation: range-only, type-only, and both.}
...
\begin{table}[H]
  \centering
  \caption[Schema Source Ablation]{Contribution of hand-curated and auto-mined schema sources.}
...
\begin{table}[H]
  \centering
  \caption[DFS Path Cap Ablation]{Effect of the DFS max paths threshold on accuracy.}
...
\begin{table}[H]
  \centering
  \caption[SIR Cross-Dataset Stability]{SIR stability across datasets and sample sizes.}
...
\begin{table}[H]
  \centering
  \caption[Filtering Impact by Dataset]{Accuracy cost of TypeOracle filtering across datasets.}
...
\begin{table}[H]
  \centering
  \caption[Beam Width Sensitivity]{Sensitivity of Hits@1 to beam width $k$.}
...
\begin{table}[H]
  \centering
  \caption[Index Path Length Sensitivity]{Sensitivity of Hits@1 to index path length.}
...
\begin{table}[H]
  \centering
  \caption[Diversity Penalty Sensitivity]{Sensitivity of Hits@1 to diversity penalty in group-beam search.}
...
\begin{table}[H]
  \centering
  \caption[Type Inference Quality Sensitivity]{Impact of type inference quality on TypeOracle performance.}
...
\begin{table}[H]
  \centering
  \caption[Time Breakdown by Phase]{Wall-clock time breakdown by pipeline phase (WebQSP, 1,627 samples).}
...
\begin{table}[H]
  \centering
  \caption[Memory Analysis]{Memory consumption by component.}
  \label{tab:memory}
...
\begin{table}[H]
  \centering
  \caption[Throughput by Method]{Throughput comparison across methods and datasets.}
...
\begin{table}[H]
  \centering
  \caption[Main Results on WebQSP]{Performance comparison on WebQSP. Our reproduced GCR baseline uses beam search ($k{=}10$) with single-stage direct extraction; the published GCR uses a two-stage pipeline with ChatGPT.}
...
\begin{table}[H]
  \centering
  \caption[GCR vs.\ DCA-Trie Multi-Dimensional Comparison]{Multi-dimensional comparison of GCR and DCA-Trie across evaluation criteria.}
...
\begin{table}[H]
  \centering
  \caption[Comparison with Related Methods]{Comparison of DCA-Trie with related graph-constrained decoding methods.}
