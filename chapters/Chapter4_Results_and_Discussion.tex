...
Following the GCR evaluation protocol~\citep{luo2025-graph-constrained-reasoning}, we evaluate on two benchmark KGQA datasets. Freebase~\citep{bollacker2008freebase} is the knowledge graph for both.
...
We compare the configurations specified in Chapter~3, Section~\ref{sec:ch3baseline}: an unconstrained CoT baseline, the GCR static-trie baseline, and the DCA-Trie v1, v2, v2-no-gates, and v3 variants. All constrained settings use the same \modelname{} backbone, the same entity-linking pipeline, and matched decoding settings (beam width $k=10$, index length $L=2$, maximum 256 new tokens). This isolates differences to the constraint-oracle design rather than model or preprocessing variation. Unlike the original GCR implementation, we do not fine-tune the KG-specialized LLM; all accuracy differences reflect oracle design changes rather than parameter changes.

\subsection{Evaluation Metrics}
\label{sec:ch4metrics}

Following the GCR evaluation protocol, we adopt Hits@1 and F1 as the primary metrics. Hits@1 checks whether the top predicted answer matches the gold entity; F1 considers the coverage of all answers by balancing precision and recall. We also report:
...
The pipeline follows: Dataset (HuggingFace Arrow) $\to$ Graph (NetworkX DiGraph) $\to$ DFS Paths $\to$ TypeOracle Gates $\to$ Filtered Paths $\to$ MarisaTrie $\to$ Constrained Decoding $\to$ Answer Extraction $\to$ Hits@1. Each dataset entry's \texttt{graph} field is a list of $[\text{head}, \text{relation}, \text{tail}]$ triples building a directed NetworkX graph of 50--500 triples per question. Depth-limited DFS from topic entities to depth \texttt{index\_len} (2 hops) is capped at 50,000 paths per question using a set for deduplication.

Path strings are wrapped with \texttt{<PATH>} prefix and \texttt{</PATH>} suffix, tokenized, and stored in a \texttt{marisa\_trie.Trie} character-level prefix tree mapping token ID prefixes to valid next-token sets. The \texttt{prefix\_allowed\_tokens\_fn} callback intercepts each generation step: before \texttt{<PATH>} all tokens are allowed, within the trie only tokens that continue a valid prefix, and after \texttt{</PATH>} all tokens resume. If the trie returns an empty set, it falls back to all tokens.

The TypeOracle is built per-question from the graph subgraph. It draws on two information sources: a hand-curated schema of 40 Freebase relations with known domain/range constraints, and an auto-mined schema inferred from data triples by aggregating entity types per relation. Two gates are applied. The \texttt{range\_gate(relation, tail\_entity)} checks whether the tail entity type intersects the relation's declared range at every hop. The \texttt{type\_gate(entity,\allowbreak answer\_types,\allowbreak hop,\allowbreak max\_hop)} checks whether the entity type intersects inferred answer types at the terminal hop only. Both use $O(1)$ set intersection over frozen sets of type strings, and default to ``allow'' when information is missing, so filtering occurs only with positive evidence of a type mismatch.

Answer types are inferred by a regex-based classifier mapping question words to expected Freebase types (``who'' $\to$ Person, ``where'' $\to$ Location, ``when'' $\to$ Date/Time, ``language'' $\to$ Human Language, and so on). The 19-pattern classifier achieves approximately 85\% recall on WebQSP and 65\% on CWQ; when no regex matches, the fallback uses the entity types of terminal entities from the path set itself.
...
Beam search is the single largest accuracy lever. The type gate performs most of the pruning (10.6\% of the 14.5\% full-set SIR reduction, versus 3.8\% for the range gate); the two schema sources are complementary, hand-curated being cleaner and auto-mined broader. The 50,000-path DFS cap affects only $\sim$2\% of questions. The gate split was measured on the full test set at $L{=}4$, where the total reduction is 14.5\%; on the controlled subset at $L{=}2$ the total is 13.8\%.

\subsubsection{Robustness and Sensitivity}

The TypeOracle SIR is stable across datasets: 13.8\% on the controlled WebQSP subset, 14.5\% on the full test set at $L{=}4$, and 13.2\% on the controlled CWQ subset. Smaller samples ($N{<}100$) overestimate SIR slightly (15.9\%), so we recommend $N{\geq}300$ for controlled comparisons. Diminishing returns set in after $k{=}10$ beam width (+11.0pp vs.\ +11.3pp at $k{=}20$), and 2-hop index length is correct for WebQSP since gold paths lie at depth $\leq 2$.
...
Table~\ref{tab:efficiency_summary} compares the three conditions' runtime and memory footprint.
...
LLM decoding dominates at 58\% of total time. DCA-Trie v1 adds 176 seconds (7.2\% overhead), with the TypeOracle gates completing in under 1 second on CPU during the DFS loop. The v3 lazy variant is the fastest of the constrained methods because it never builds the full trie; its slightly higher peak VRAM (16.6 vs.\ 16.0\,GB) reflects the frontier-tracking structures, not a larger path table.
...
Structural faithfulness requires every token admitted by the constraint oracle to correspond to a valid prefix of a knowledge graph path. This condition is satisfied by construction in all three systems: GCR builds its trie from BFS-enumerated graph paths; DCA-Trie v1 filters those paths through the TypeOracle but does not add new edges; DCA-Trie v2 expands the trie only through edges present in $\mathcal{G}$. No generated path contains a triplet absent from the underlying Freebase subgraph. The guarantee holds because the logit mask $\mathbf{m}_t$ is derived from trie lookup, and the trie is populated exclusively from valid graph edges. The TypeOracle changes \textit{which} valid edges are admitted, not \textit{whether} admitted edges are valid. Condition~F is therefore satisfied for all methods.

\subsection{Condition R: Semantic Relevance Reduction}

The TypeOracle reduces the candidate path set from 790,984 to 682,222 total paths (2,637 to 2,274 per question), a 13.8\% reduction on the controlled 300-question subset.

The 13.8\% reduction represents the fraction of structurally valid paths that the TypeOracle judged semantically irrelevant. These paths were reachable through valid edges, but either their relation ranges or their terminal entity types were incompatible with the question's intent. The corpus-level SIR is 0.138: roughly one in seven admitted paths is semantically irrelevant. For GCR's static oracle, the SIR is 1.0 by definition.

These two numbers, 13.8\% and the SIR drop from 1.0 to 0.138, describe different statistics of the same result. The 13.8\% figure is the drop in total path count (790,984 to 682,222). The SIR drop is the drop in the fraction of admitted paths that are semantically irrelevant, equivalently an 86.2\% relative reduction in irrelevance: GCR's unfiltered trie is 100\% irrelevant by definition (SIR~=~1.0), while DCA-Trie's filtered trie is only 13.8\% irrelevant (SIR~=~0.138). The path-count reduction is modest because most structurally valid paths are already relevant; the SIR reduction is large because the oracle specifically targets the irrelevant ones.

The total reduction decomposes into two independent gate contributions, as shown in Table~\ref{tab:gates}. The decomposition was measured on the full test set at index length $L{=}4$, where the total reduction reaches 14.5\%; the gate split is the same gate logic applied to the larger path space.

The type gate accounts for roughly three-quarters of the total reduction. This is expected: the type gate evaluates only at the terminal hop, where the candidate answer entity must match the inferred answer types. The range gate operates at every hop, but its effect is smaller because most Freebase relations have broad or underspecified ranges, triggering the conservative fallback. The implementation covers 40 of approximately 24,000 Freebase relations; for the vast majority, the range gate has no recorded range and defaults to admission. Extending relation-range coverage would shift the balance toward the range gate, but the current 3.8\% figure is a lower bound on what a more complete ontology could achieve.

\subsection{Condition P: Recall Preservation}

Table~\ref{tab:gates} reports both the gate decomposition and the false negative rates for each TypeOracle gate; the FNR figures use the WebQSP validation split.

\begin{table}[H]
  \centering
  \caption[TypeOracle Gate Analysis]{TypeOracle Gate Analysis}
  \label{tab:gates}
  \begin{tabularx}{\textwidth}{|X|r|r|r|r|}
    \hline
    \textbf{Gate} & \textbf{Paths Blocked} & \textbf{\% of Total} & \textbf{Gold Paths Excluded} & \textbf{FNR} \\
    \hline
    Type gate     & 435,897                & 10.6\%               & 490 / 14,829                 & 3.3\%        \\
    \hline
    Range gate    & 157,485                & 3.8\%                & 424 / 14,829                 & 2.9\%        \\
    \hline
...
Both gates individually satisfy Condition~P (FNR $< 5\%$). The type gate excludes 3.3\% of gold paths, and the range gate excludes 2.9\%. The excluded gold paths typically involve questions where the answer entity's Freebase type is incomplete or where the relation's declared range does not cover the actual answer.
...
Table~\ref{tab:main_results} presents the main results on the WebQSP benchmark, measured on the controlled 300-question subset (seed 42) under beam search ($k{=}10$) with single-stage direct extraction. Our reproduced GCR baseline achieves 85.7\% Hits@1. The published 92.6\% row is reported for reference only and is \emph{not} directly comparable to the rows below it: it measures a different subset ($L{=}4$ index, full test set) and a different pipeline (ChatGPT second stage). The reproduction gap is analysed in Section~\ref{sec:ch4repro}; how DCA-Trie would fare under the official two-stage pipeline is a protocol question, not a constraint-quality one. The DCA-Trie rows are all paired against the reproduced baseline on the identical 300 questions, which is the comparison that supports the non-monotone claim.
...
Four mechanisms contribute to the non-monotone behaviour: gold-path exclusion (the type-gate FNR of Section~\ref{sec:ch4conditions} removes some correct answers before decoding begins), beam competition (with a smaller trie, beam search has fewer diverse candidates, and a path ranked second or third in the larger GCR trie may not survive pruning), type-inference noise (when inferred types are wrong, the type gate removes correct paths), and the precision-recall trade-off (the oracle improves precision but hurts recall more, so the net effect on Hits@1 is negative). The v2 collapse is a fifth, distinct mechanism analysed in Section~\ref{sec:ch4v2}; it is architectural, not a product of the gates.

\subsection{The Type-Inference Confound}

A skeptical reader could attribute the non-monotone result to the quality of the type-inference heuristic rather than to a genuine tightness-accuracy decoupling: if the regex classifier were perfect, the TypeOracle might improve accuracy. This objection has merit. The 19-pattern regex classifier is the weakest component of the pipeline, and Case~1 in Section~\ref{sec:ch4cases} shows a concrete failure where misclassification directly causes an incorrect answer.

The confound does not fully explain the result. The type gate's false negative rate is below 5\% (Table~\ref{tab:gates}), which accounts for only a fraction of the 4.0pp accuracy drop. The remainder comes from beam-competition effects and precision-recall tradeoffs independent of type-inference quality. Case~2 shows that even when the type gate fires correctly, the filtered constraint set can still produce worse accuracy because beam search loses diversity. The SIR reduction from 1.0 to 0.138 shows the filtering is directionally correct: the problem is not that the oracle filters the wrong paths, but that the decoding process needs some of those irrelevant paths to maintain beam diversity. The cleanest evidence is the calibration run in Section~\ref{sec:ch4v3}: with the gates disabled, the lazy constraint reproduces the baseline exactly, so every gated-number difference is attributable to the gates themselves.
...
Table~\ref{tab:timing} compares wall-clock execution times for all methods.
...
DCA-Trie v1 adds approximately 176 seconds (7.2\%) to the total runtime relative to GCR, from the TypeOracle filtering pass running in $O(|\mathcal{P}| \cdot L)$ time. The per-question average is nearly identical because the $O(1)$ set lookups are negligible compared to LLM decoding time, and the pass completes in under 1 second on CPU while the GPU is occupied. The v2 variants are \textit{faster} than baseline because the per-hop trie is far smaller, yet they lose 25 points of accuracy; wall-clock time is not a proxy for constraint quality. The v3 lazy run is nearly as fast as baseline (7.92s vs.\ 8.12s per question) while materialising only a fraction of the path space. Compared to the GCR paper's efficiency analysis (Table~2 in \citet{luo2025-graph-constrained-reasoning}), DCA-Trie preserves the same profile: no additional LLM calls or input tokens.
...
The three cases illustrate distinct failure modes: Case~1 shows the question-type heuristic introducing errors that propagate through the gate logic, Case~2 shows the oracle correctly filtering an irrelevant path, and Case~3 shows that even correct filtering can harm beam-search performance by reducing diversity. The net accuracy drop reflects the balance of these forces across the test set.
...
To test whether the non-monotone finding transfers to more complex questions, we run a controlled 300-question CWQ evaluation (2--4 hops) matching the WebQSP protocol: the GCR baseline, DCA-Trie v1, v3, and the v3-no-gates calibration control decode the identical question set sampled with seed~42. Table~\ref{tab:cwq_results} presents the results.
...
Table~\ref{tab:katamanso_criterion} summarises the multi-dimensional comparison between GCR and DCA-Trie. No method dominates across all dimensions; the choice depends on which properties the task prioritises.
...
GCR leads on accuracy; DCA-Trie leads on search-space quality, semantic filtering, and analytical interpretability. Across the methods reviewed in Section~\ref{sec:constrained}, DCA-Trie is the only framework that combines structural faithfulness, dynamic expansion (v2), and semantic filtering in a single oracle, applying type-theoretic constraints derived from the Freebase ontology rather than learning relevance from data.

The experimental results support four findings. First, the TypeOracle satisfies its design conditions (Section~\ref{sec:ch4conditions}). Second, those conditions are satisfied without improving answer accuracy: the oracle removes genuinely irrelevant paths (SIR falls from 1.0 to 0.138) but the decoding process relies on some of those paths to maintain beam diversity, so Hits@1 drops 4.0pp (v1) and 4.7pp (v3). Tightness and accuracy are independent properties; this is the thesis's core contribution, since the permissiveness problem identified in Chapter~2, while real, is not the primary driver of error. Third, the v2 collapse is architectural and gate-independent (Section~\ref{sec:ch4v2}). Fourth, the non-monotone relationship forces a re-evaluation of the field's implicit assumption: in classical planning a smaller search space converges faster, but autoregressive decoding does not work this way. The search space interacts with beam-search pruning and the model's probability distribution, so reducing it removes distractors while also reducing diversity and, when type-inference noise is present, excluding correct answers.

The implication is clear: future work should not treat constraint tightness as a proxy for constraint quality. An oracle that admits fewer paths is not necessarily better; what matters is whether it admits the \textit{right} paths. The oracle hierarchy (structural $\to$ ontology $\to$ question $\to$ useful) is not a ladder to climb but a landscape to navigate, and the filtering-unfiltering tradeoff is its central axis: every admission decision either removes noise or removes diversity, with the balance varying by question.

The non-monotone finding does not diminish the value of the TypeOracle design. It reveals that the relationship between constraint quality and answer quality is more nuanced than previously assumed. The TypeOracle satisfies its design conditions, maintains 100\% structural faithfulness, and adds negligible overhead; the accuracy drop is a consequence of the interaction between semantic filtering and beam-search dynamics, not a flaw in the oracle itself.
...
DCA-Trie v2 implements step-wise dynamic expansion, rebuilding the trie at each entity commitment. Table~\ref{tab:main_results} reports its WebQSP result: 60.7\% Hits@1, substantially worse than both the GCR baseline (85.7\%) and DCA-Trie v1 (81.7\%). v2 was not run on CWQ.
