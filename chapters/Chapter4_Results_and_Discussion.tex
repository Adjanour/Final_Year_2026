...
The total reduction decomposes into two independent gate contributions, as shown in Table~\ref{tab:gate_decomp}. The decomposition was measured on the full test set at index length $L{=}4$, where the total reduction reaches 14.5\%; the gate split is the same gate logic applied to the larger path space.
...
GCR achieves 100\% faithful reasoning ratio on both WebQSP and CWQ: every generated path can be found in the knowledge graph~\citep{luo2025-graph-constrained-reasoning}. DCA-Trie inherits this property by construction, as Table~\ref{tab:faithful} confirms.
...
GCR leads on accuracy; DCA-Trie leads on search-space quality, semantic filtering, and analytical interpretability. Table~\ref{tab:method_comparison} positions DCA-Trie against related constrained-decoding methods. DCA-Trie is the only method that combines dynamic constraint adaptation with a symbolic oracle operating over the full KG schema.
