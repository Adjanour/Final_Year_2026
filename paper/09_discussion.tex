...
rebuilding is an architectural risk that costs far more than any semantic filtering.
...
The non-monotone finding generalises beyond WebQSP. A controlled 300-question CWQ run
using the same paired protocol (baseline, v1, v3, and the v3-no-gates calibration
control) reproduces every result: v1 drops 5.3pp Hits@1 (43.7\% vs.\ 49.0\%, McNemar
$p=0.002$), v3 stays within noise ($-1.0$pp, $p=0.37$) while remaining the fastest
condition (9.17\,s vs.\ 9.64\,s per question), and v3 without gates reproduces the
baseline exactly (100\% per-question agreement, $p=1.0$). SIR is stable across datasets
(0.132 on CWQ vs.\ 0.138 on WebQSP). Zero-shot transfer beyond Freebase remains future
work.
